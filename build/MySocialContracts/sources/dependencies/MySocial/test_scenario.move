...
/// Utility for mocking a multi-transaction Mys execution in a single Move procedure.
...
public fun return_to_address<T: key>(account: address, t: T) {
    ...
    mys::transfer::transfer_impl(t, account)
}
...
public fun return_immutable<T: key>(t: T) {
    ...
    mys::transfer::freeze_object_impl(t)
}
...
public fun return_shared<T: key>(t: T) {
    ...
    mys::transfer::share_object_impl(t)
}
...
public fun most_recent_receiving_ticket<T: key>(owner: &ID): mys::transfer::Receiving<T> {
...
public fun receiving_ticket_by_id<T: key>(object_id: ID): mys::transfer::Receiving<T> {
    let version = allocate_receiving_ticket_for_object<T>(object_id);
    mys::transfer::make_receiver(object_id, version)
}
...
/// test scenario.
public fun return_receiving_ticket<T: key>(ticket: mys::transfer::Receiving<T>) {
    let id = mys::transfer::receiving_id(&ticket);
