...
/// `list_with_purchase_cap` therefore performing a trade (ismysng a
...
entry fun default(ctx: &mut TxContext) {
    ...
    mys::transfer::transfer(cap, ctx.sender());
    mys::transfer::share_object(kiosk);
}
...
public fun close_and_withdraw(self: Kiosk, cap: KioskOwnerCap, ctx: &mut TxContext): Coin<MYS> {
...
public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<MYS> {
    assert!(self.has_access(cap), ENotOwner);
    &mut self.profits
}
