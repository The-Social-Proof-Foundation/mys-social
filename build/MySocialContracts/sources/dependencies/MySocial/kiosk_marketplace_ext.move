...
#[test_only]
module mys::kiosk_marketplace_ext {
    use mys::mys::MYS;
    use mys::coin::Coin;
    use mys::kiosk_extension as ext;
    use mys::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use mys::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of MYS.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in MYS) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(
        kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<MYS>
    ) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<MYS> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module mys::kiosk_extensions_tests {
    use mys::kiosk_test_utils::{Self as test};
    use mys::kiosk_extension as ext;
    ...
    #[test, expected_failure(abort_code = mys::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = mys::kiosk_extension::EExtensionNotInstalled)]
