...
module mys::pay_tests {
    ...
    fun test_coin_split_n() {
        ...
        let mut coin = coin::mint_for_testing<MYS>(10, ctx);

        scenario.next_tx(TEST_SENDER_ADDR);
        coin.divide_and_keep(3, scenario.ctx());

        scenario.next_tx(TEST_SENDER_ADDR);
        let coin1 = scenario.take_from_sender<Coin<MYS>>();

        scenario.next_tx(TEST_SENDER_ADDR);
        let coin2 = scenario.take_from_sender<Coin<MYS>>();
        ...
        assert!(coin.value() == 4);
        assert!(
            !scenario.has_most_recent_for_sender<Coin<MYS>>(),
            1
        );
    ...
    fun test_split_and_transfer_fail() {
        ...
        scenario.next_tx(TEST_SENDER_ADDR);
        let coin_transfer_fail = scenario.take_from_sender<Coin<MYS>>();
