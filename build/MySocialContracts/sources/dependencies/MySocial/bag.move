...
/// A bag is a heterogeneous map-like collection. The collection is similar to `mys::table` in that
/// its keys and values are not stored within the `Bag` value, but instead are stored using Mys's
...
/// assert!(&bag1 != &bag2);
/// ```
/// At it's core, `mys::bag` is a wrapper around `UID` that allows for access to
/// `mys::dynamic_field` while preventing accidentally stranding field values. A `UID` can be
...
/// Aborts with `mys::dynamic_field::EFieldAlreadyExists` if the bag already has an entry with
/// that key `k: K`.
...
/// Aborts with `mys::dynamic_field::EFieldDoesNotExist` if the bag does not have an entry with
/// that key `k: K`.
/// Aborts with `mys::dynamic_field::EFieldTypeMismatch` if the bag has an entry for the key, but
